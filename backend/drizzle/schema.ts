import { pgTable, index, unique, serial, text, integer, boolean, timestamp, foreignKey, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const role = pgEnum("role", ['user', 'admin', 'employee', 'media-handler'])
...
export const hotelRoomReservation = pgTable("hotelRoomReservation", {
	...
	index("hotelRoomReservation_email_idx").using("btree", table.email.asc().nullsLast().op("text_ops")),
	unique("hotelRoomReservation_application_id_unique").on(table.applicationId),
]);

export const adminEventTable = pgTable("adminEventTable", {
	...
	unique("adminFoodCourtTable_table_name_key").on(table.eventId),
]);
...
export const ticket = pgTable("ticket", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "ticket_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	name: text().notNull(),
	email: text().notNull(),
	phone: text().notNull(),
...
export const announcements = pgTable("announcements", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "announcements_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	title: text(),
	body: text(),
	displayType: text(),
	image: text(),
