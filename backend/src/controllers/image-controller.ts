...
export const getImage = async (c: Context) => {
	try {
		...
		if (param.toLowerCase().includes("hotel")) {
			...
			const hotelGallery = await cloudService.listByMetadata(
				"position",
				"gallery-hotel",
				"gallery",
			);
			...
			const foodCourtGallery = await cloudService.listByMetadata(
				"position",
				"gallery-food-court",
				"gallery",
			);
			...
			const bakeryGallery = await cloudService.listByMetadata(
				"position",
				"gallery-bakery",
				"gallery",
			);
			...
			bakeryImages.forEach((img) => {
				...
				const desc =
					img.context?.description ?? "Description not available";
