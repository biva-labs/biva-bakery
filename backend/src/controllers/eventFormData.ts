...
export const eventFormData = async (c: Context) => {
  try {
    ...
    if (imgFile instanceof File) {
      try {
        uploadedImage = await uploadImage(
          imgFile,
          "officialDocumentImageForVisitors",
        );
      } catch (error) {
        console.error(
          "Image upload filed: this is eventformdata.ts line 30",
          error,
        );
    ...
    return c.json(
      { message: "form submitted sucess", data: insertedData },
      201,
    );
  } catch (error) {
