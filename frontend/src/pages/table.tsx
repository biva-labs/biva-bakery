...
export default function Table() {
  ...
  const handleBookAndPay = async () => {
    ...
    console.log("Store state before submission:", {
      ...
      adhaar_or_pan_card: data.adhaar_or_pan_card ? "File present" : "No file",
    });

    if (!data.name || !data.email || !data.event_id || !data.number_of_guest) {
    ...
    submitForm(
      ...
      },
      {
        onSuccess: async (response) => {
          ...
          const user = response.data?.data?.insertedData.map((v) => v.id);
          const eventType = response.data?.data?.insertedData[0].eventId;
          console.log("AFTER PAYMENT DATA:", user);
          await initiatePayment(totalAmount, user, eventType);
        },
        onError: (error) => {
          ...
          <div className="w-full lg:w-1/2 space-y-6">
            ...
            {parseInt(data.number_of_guest) > 0 && (
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-900">
                  Guest Information ({data.number_of_guest} guests)
                </h3>
                {Array.from(
                  ...
                  (_, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <GuestEventSeatForm guestId={guestId} />
                      </div>
                    );
                  },
                )}
            ...
            >
              {isPending || isProcessing
                ? "Processing..."
                : (() => {
                    const guests = parseInt(data.number_of_guest) || 0;
                    const total =
                      (guests + 1) * parseInt(eventData.price || "0");
                  ...
                  .map(([key, value]) => (
                    <div key={key} className="space-y-2">
                      <Label className="text-sm font-medium text-blue-800">
                        {KEY_VALUE_MAP[key as keyof typeof KEY_VALUE_MAP]}
                      </Label>
                      <Input
                        value={value}
                        disabled={true}
