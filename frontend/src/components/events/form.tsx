      ...
      <div key={field.id} className="space-y-2 ">
        ...
        {field.element === "input" ? (
          <Input
            ...
            className={field.disabled ? "bg-gray-100 text-gray-600" : ""}
          ...
          <div className="space-y-2">
            ...
            />
            {fieldValue && (
              <p className="text-sm text-green-600">Selected: {fieldValue}</p>
            )}
          </div>
            ...
            <SelectContent>
              {field.options?.map((option: any, index: number) => (
                <SelectItem key={index} value={String(option)}>
                  {String(option)} {field.id === "guest-no" ? "guests" : ""}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
              ...
              <div className="flex flex-wrap gap-2">
                {(fieldValue as string[]).map((seatId) => (
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      toggleSeatSelection(seatId, fieldValue as string[])
                    }
    ...
    <div className="w-full mt-5 space-y-6 h-full">
      <div className="p-4 md:p-6 rounded-lg  ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-1 gap-4">
          {EVENT_FORM_FIELDS.slice(6).map((field) => renderField(field))}
        </div>
      </div>
    </div>
  );
